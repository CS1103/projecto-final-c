...
using Tensor = utec::algebra::Tensor<T, DIMS>;
    ...
    private:
        ...
        long t = 0;
        std::unique_ptr<utec::algebra::Tensor<T,2>> m, v;
...
void update(utec::algebra::Tensor<T,2>& params,
            ...
            if (!initialized || (m && m->shape() != params.shape())) {
                m = std::make_unique<utec::algebra::Tensor<T,2>>(params.shape());
                v = std::make_unique<utec::algebra::Tensor<T,2>>(params.shape());
                m->fill(0);
                v->fill(0);
                initialized = true;
                t = 0;
            ...
            for (size_t i = 0; i < params.shape()[0]; ++i) {
                for (size_t j = 0; j < params.shape()[1]; ++j) {
                    ...
                    v_hat = std::max(v_hat, T(1e-12)); // Protección contra nan
        ...
        void step() override {
            // Implementación opcional, puede quedar vacía si no la usas
        }
    };
} // namespace utec::neural_network
